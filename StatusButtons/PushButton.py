...
class StatusPushButton(QtWidgets.QWidget):
...
# "}\n"
"\n")
        self.setObjectName("statusButtonFrame")
        ...
        self.statusButton.setMinimumSize(QtCore.QSize(0, 30))
        ...
        self.statusButton.setText(initial_text)

        # self.statusButton.setSizeAdjustPolicy(QtWidgets.QComboBox.AdjustToContents)
        self.statusButton.setObjectName("statusButton")
        self.__layout.addWidget(self.statusButton)
    
    def setStatusIdle(self, text :str = None) -> None: 
        self.__dot.setStyleSheet( 
            self.DOT_STYLE.replace('{OPTION}', '#B5BFC6') 
        )
        if text is not None: self.statusButton.setText(text)
    def setStatusConnected(self, text :str = None) -> None: 
        self.__dot.setStyleSheet( 
            self.DOT_STYLE.replace('{OPTION}', '#04B56E') 
        )
        if text is not None: self.statusButton.setText(text)
    def setStatusConnecting(self, text :str = None) -> None: 
        self.__dot.setStyleSheet( 
            self.DOT_STYLE.replace('{OPTION}', '#F99500') 
        )
        if text is not None: self.statusButton.setText(text)
    def setStatusDisconnected(self, text :str = None) -> None: 
        self.__dot.setStyleSheet( 
...
if __name__ == '__main__':
    ...
    #TUTORIAL ######################################
    statusBox1 = StatusPushButton(window)
    
    statusBox1.statusButton.clicked.connect(print) # Print the index after comboBox get clicked

    statusBox2 = StatusPushButton(window)
    statusBox2.setStatusConnecting(text="Connecting")

    statusBox3 = StatusPushButton(window)
    statusBox3.setStatusConnected(text="Connected")

    statusBox4 = StatusPushButton(window)
    statusBox4.setStatusDisconnected(text="Error")
